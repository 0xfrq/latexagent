...
editor latex, interactive pdf viewer, dan revision section
"""
...
def render(current_topic, config):
    ...
    with col_viewer:
        ...
        if cached_pdf:
            ...
            render_viewer(pages_data, pdf_b64, height=800, key=f"viewer_{current_topic['id']}")
        else:
            st.info("tekan Compile untuk melihat PDF")

    # =============================================
    # revision section (minimal)
    # =============================================
    st.markdown("---")
    st.markdown("#### Revise Section")
    st.caption("tandai area di PDF pakai tool 'Select to Revise', lalu isi di sini apa yang mau diubah. ai akan mengubah HANYA bagian itu.")
    ...
    with col_rb1:
        apply_rev = st.button("Apply Revision", use_container_width=True)
    with col_rb2:
    ...
    if current_topic.get("revision_history"):
        with st.expander(f"Revision History ({len(current_topic['revision_history'])} changes)"):
            for i, rev in enumerate(current_topic["revision_history"]):
                st.markdown(
                    f"**#{i+1}** [{rev['timestamp']}] "
                    f"**Page {rev.get('page', '?')}**: {rev['request']}"
                )
                st.divider()


def _handle_revision(current_topic, rc, latex_input, cached_pdf, page_num, revision_desc):
    ...
    # extract teks dari halaman buat konteks ai
    page_text = ""
    if cached_pdf:
        page_text = extract_page_text(cached_pdf, page_num - 1)

    section_ctx = f"content on page {page_num}"
    ...
    with st.spinner(f"ai merevisi bagian di halaman {page_num}..."):
        try:
            ...
            if sukses:
                topics.save_pdf(current_topic["id"], pdf_bytes)
                st.success(f"revisi halaman {page_num} berhasil!")
            else:
                st.warning("revisi diterapkan tapi kompilasi gagal.")
